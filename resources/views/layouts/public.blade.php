    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .navbar{display:flex;justify-content:space-between;align-items:center;
                padding:14px 6%;background:var(--pink-blush);
                box-shadow:0 2px 14px rgba(0,0,0,.06);
                position:sticky;top:0;z-index:1000}

        /* Logo */
        .logo-wrap{display:flex;align-items:center;gap:12px;text-decoration:none;flex-shrink:0}
        .logo-img{width:46px;height:46px;border-radius:50%;border:2.5px solid var(--pink);object-fit:cover}
        .logo-fallback{width:46px;height:46px;border-radius:50%;background:var(--pink);
        ...
        /* WA Button */
        .nav-wa{display:flex;align-items:center;gap:8px;background:var(--pink);
                color:#fff!important;padding:10px 22px;border-radius:22px;
                font-size:13px;font-weight:600;text-decoration:none;
                box-shadow:0 4px 14px rgba(242,117,117,.28);
                transition:background var(--t),transform var(--t)}
        .nav-wa::after{display:none!important}
        .nav-wa:hover{background:var(--pink-dark)!important;transform:translateY(-1px)}

        /* Hamburger */
        .hamburger{display:none;flex-direction:column;gap:5px;background:none;
                   border:none;padding:6px;cursor:pointer}
        .hamburger span{display:block;width:24px;height:2px;background:var(--pink);
        ...
        /* Mobile nav */
        .mobile-nav{display:none;flex-direction:column;position:absolute;top:100%;
                    left:0;right:0;background:var(--pink-blush);
                    border-top:1px solid var(--border);border-bottom:2px solid var(--pink-light);
                    padding:8px 6% 16px;gap:0;box-shadow:0 8px 20px rgba(0,0,0,.08)}
        ...
        /* ── FOOTER ── */
        footer{background:#1a1a1a;color:#999;padding:64px 6% 0}
        ...
        .footer-col .fc-item{display:flex;align-items:flex-start;gap:10px;color:#bbb;
                             font-size:14px;margin-bottom:11px;line-height:1.5;text-decoration:none}
        .footer-col .fc-item:hover{color:var(--pink)}
        .footer-col .fc-item i{color:var(--pink);width:16px;flex-shrink:0;margin-top:2px}
        .footer-desc{color:#bbb;font-size:14px;line-height:1.8;margin-bottom:16px}
        .social-row{display:flex;gap:10px;margin-top:18px}
        .social-btn{width:38px;height:38px;border-radius:50%;background:#2d2d2d;
                    display:flex;align-items:center;justify-content:center;
                    color:#bbb;font-size:15px;text-decoration:none;
                    transition:background var(--t),color var(--t),transform var(--t)}
        .social-btn:hover{background:var(--pink);color:#fff;transform:translateY(-2px)}
        .footer-bottom{border-top:1px solid #2d2d2d;padding:22px 0;
                       text-align:center;font-size:13px;color:#555}

        /* ── UTILITIES ── */
        .btn-pink{display:inline-flex;align-items:center;gap:8px;background:var(--pink);
                  color:#fff;padding:13px 30px;border-radius:24px;text-decoration:none;
                  font-size:15px;font-weight:600;box-shadow:0 6px 20px rgba(242,117,117,.28);
                  transition:background var(--t),transform var(--t)}
        ...
        .btn-outline{display:inline-flex;align-items:center;gap:8px;background:transparent;
                     color:var(--pink);padding:12px 28px;border-radius:24px;text-decoration:none;
                     font-size:15px;font-weight:600;border:2px solid var(--pink);
                     transition:background var(--t),color var(--t)}
        .btn-outline:hover{background:var(--pink);color:#fff}
        .badge-kateg{display:inline-block;background:var(--pink-pale);color:var(--pink);
...
(function(){
    var btn=document.getElementById('hamburgerBtn'),
        ...
        isOpen=false;
    function open(){isOpen=true;nav.classList.add('open');btn.classList.add('open');
        btn.setAttribute('aria-expanded','true');btn.setAttribute('aria-label','Tutup menu')}
    function close(){isOpen=false;nav.classList.remove('open');btn.classList.remove('open');
