...
<head>
    ...
    <style>
        *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
        ...
        .tentang-hero { height:80vh; background-color:#d95f5f;
                        background-size:cover; background-position:center;
                        background-repeat:no-repeat; display:flex;
                        flex-direction:column; justify-content:center;
                        align-items:center; position:relative;
                        text-align:center; color:#fff; }
        .tentang-hero::after { content:''; position:absolute; bottom:0; left:0; right:0;
                               height:180px;
                               background:linear-gradient(to bottom,rgba(255,192,203,0),#FFC0CB); }
        .tentang-hero h1 { font-size:56px; font-weight:bold;
                           text-shadow:2px 2px 15px rgba(0,0,0,.5);
                           position:relative; z-index:2; }
        .tentang-hero p  { font-size:17px; margin-top:12px; opacity:.9;
                           position:relative; z-index:2; }

        .visimisi-section { padding:80px 10%; background:#fff; text-align:center; }
        .visimisi-section h2 { font-size:32px; color:#F27575; font-weight:bold; margin-bottom:12px; }
        .visimisi-section p.sub { font-size:15px; color:#888; margin-bottom:56px; }
        .visimisi-grid { display:grid; grid-template-columns:1fr 1fr; gap:32px;
                         max-width:960px; margin:0 auto; }
        .vm-card { background:#fff; border-radius:24px; padding:40px 36px;
                   text-align:left; box-shadow:0 12px 40px rgba(242,117,117,.12);
                   border-top:5px solid #F27575; }
        .vm-card.misi { border-top-color:#FFC6C9; }
        .vm-card .vm-icon { font-size:2.6rem; margin-bottom:20px; display:block; }
        .vm-card h3 { font-size:22px; font-weight:800; color:#F27575; margin-bottom:16px; }
        .vm-card.misi h3 { color:#d95f5f; }
        .vm-card p  { font-size:15px; color:#555; line-height:1.85; }
        .vm-card ul { list-style:none; padding:0; margin:0;
                      display:flex; flex-direction:column; gap:10px; }
        .vm-card ul li { display:flex; align-items:flex-start; gap:10px;
                         font-size:14px; color:#555; line-height:1.7; }
        .vm-card ul li::before { content:'●'; color:#F27575; font-size:11px;
                                 flex-shrink:0; margin-top:4px; }

        .achiev-section { padding:80px 8%; background:#FFFEFA; }
        .achiev-section h2 { font-size:32px; color:#F27575; font-weight:bold;
                             text-align:center; margin-bottom:12px; }
        .achiev-section p.sub { font-size:15px; color:#888; text-align:center; margin-bottom:56px; }
        .achiev-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(200px,1fr));
                       gap:24px; max-width:1000px; margin:0 auto 56px; }
        .achiev-card { background:#fff; border-radius:20px; padding:36px 24px;
                       text-align:center; box-shadow:0 6px 24px rgba(0,0,0,.07);
                       transition:transform .25s, box-shadow .25s; }
        .achiev-card:hover { transform:translateY(-6px);
                             box-shadow:0 14px 40px rgba(242,117,117,.15); }
        .achiev-card .achiev-icon  { font-size:2.8rem; margin-bottom:16px; display:block; }
        .achiev-card .achiev-num   { font-size:36px; font-weight:900; color:#F27575;
                                     line-height:1; margin-bottom:8px; }
        .achiev-card .achiev-label { font-size:14px; color:#777; font-weight:600; }

        .timeline-wrap { max-width:760px; margin:0 auto; }
        .timeline-wrap h3 { font-size:22px; font-weight:800; color:#333;
        ...
        .info-section { background:#fff; padding:60px 8%; }
        ...
        @media(max-width:768px) {
            ...
            .tentang-hero h1 { font-size:36px; }
            .visimisi-grid { grid-template-columns:1fr; }
            .vm-card { padding:28px 22px; }
            .achiev-grid { grid-template-columns:1fr 1fr; }
            footer { flex-direction:column; }
        }
        @media(max-width:480px) {
            .achiev-grid { grid-template-columns:1fr; }
            .tentang-hero { height:45vh; }
            .tentang-hero h1 { font-size:28px; }
        }
    </style>
</head>
<body>
...
<section class="tentang-hero"
    style="background-image:linear-gradient(rgba(0,0,0,.25),rgba(0,0,0,.35)),url({{ asset('images/tentangbg.jpg') }})">
    <h1>Tentang Kami</h1>
    ...
    <div class="timeline-wrap">
        ...
        <div class="timeline">
            <div class="tl-item">
                ...
                <div class="tl-text">{{ $namaToko }} lahir dari dapur rumahan dengan 5 varian donat dan semangat besar untuk menghadirkan camilan berkualitas bagi warga sekitar.</div>
            </div>
            <div class="tl-item">
                <div class="tl-dot"></div>
                <div class="tl-year">2017 — Toko Pertama</div>
                <div class="tl-text">Membuka gerai fisik pertama dan langsung disambut antusias oleh pelanggan setia. Menu diperluas hingga 20 varian dengan topping kekinian.</div>
            </div>
            <div class="tl-item">
                <div class="tl-dot"></div>
                <div class="tl-year">2019 — Penghargaan UMKM</div>
                <div class="tl-text">Meraih penghargaan sebagai UMKM Kuliner Terbaik tingkat kota atas inovasi produk dan konsistensi kualitas bahan baku.</div>
            </div>
            <div class="tl-item">
                <div class="tl-dot"></div>
                <div class="tl-year">2021 — Hadir Online</div>
                <div class="tl-text">Meluncurkan layanan pemesanan online sehingga donat segar bisa dinikmati kapan saja dan di mana saja oleh lebih banyak pelanggan.</div>
            </div>
            <div class="tl-item">
                <div class="tl-dot"></div>
